require([
    ...
    "esri/Graphic",
    "esri/geometry/Point"
], function (Map, MapView, FeatureLayer, Expand, UniqueValueRenderer, SimpleMarkerSymbol, Graphic, Point) {
    // Configuration with Portland TX color scheme
    const config = {
        serviceUrl:
            "https://services3.arcgis.com/DAf01WuIltSLujAv/arcgis/rest/services/Portland_Meters/FeatureServer/0",
        fieldNames: {
            address: "address",
            account: "Account",
            customer: "Customer",
        ...
        colors: {
            needsUpdate: "#2247fe", // Portland TX blue
            updated: "#80c940", // Portland TX green
            selected: "#fb7f31", // Portland TX orange
        },
    };
    ...
    async function initMap() {
        ...
        view = new MapView({
            container: "viewDiv",
            map: map,
            center: [-97.3238, 27.8772], // Portland, TX coordinates
            zoom: 13,
        });

        // Renderer for Account_Update field
        ...
        // Legend widget in expand
        const legend = new esri.widgets.Legend({
            view: view,
            style: {
                type: "card",
                layout: "side-by-side",
    ...
    async function loadAllFeatures() {
        try {
            ...
            console.log(`Loaded ${allFeatures.length} meters`);

            // Update renderer with individual values if needed
            updateRenderer();
    ...
    function updateRenderer() {
        ...
        // Gather unique non-empty Account_Update values
        const updatedValues = new Set();
        allFeatures.forEach((feature) => {
            const val = feature.attributes[config.fieldNames.accountUpdate];
            if (val && val.trim() !== "") {
    ...
    function performSearch(query) {
        ...
        if (!allFeatures.length) {
            searchResults.innerHTML =
                '<div class="search-result-item">Loading data...</div>';
        ...
        if (results.length === 0) {
            searchResults.innerHTML =
                '<div class="search-result-item no-results">No meters found</div>';
        } else {
            searchResults.innerHTML = results
                .map((feature) => {
                    const address = feature.attributes[config.fieldNames.address];
                    const account = feature.attributes[config.fieldNames.account];
                    const customer = feature.attributes[config.fieldNames.customer];
                    return `
                    <div class="search-result-item" data-objectid="${feature.attributes.OBJECTID}">
                        <div class="result-main">${address || "No address"}</div>
                        <div class="result-sub">Account: ${account || "No account"} | Customer: ${
    ...
    function showMeterDetails(feature) {
        ...
        content.innerHTML = `
            <div class="details-section">
                ...
                <div class="detail-group priority">
                    <div class="detail-item">
                        <label>Address:</label>
                        <span class="detail-value address-highlight">${
                            attrs[config.fieldNames.address] || "Not specified"
                        }</span>
                    </div>
                    <div class="detail-item">
                        <label>Account Number:</label>
                        <span class="detail-value account-highlight">${
                            attrs[config.fieldNames.account] || "Not specified"
                        }</span>
                    </div>
                </div>
            </div>
                ...
                <div class="detail-group">
                    <div class="detail-item">
                        ...
                        <span class="detail-value">${
                            attrs[config.fieldNames.customer] || "Not specified"
                        }</span>
                    </div>
                    <div class="detail-item">
                        <label>Customer / Account:</label>
                        <span class="detail-value">${
                            attrs[config.fieldNames.customerAccount] || "Not specified"
                        }</span>
                    </div>
                </div>
            </div>
            <div class="details-section">
                <h3 class="section-title">⚙️ Meter Details</h3>
                <div class="detail-group">
                    <div class="detail-item">
                        <label>Class:</label>
                        <span class="detail-value">${
                            attrs[config.fieldNames.class] || "Not specified"
                        }</span>
                    </div>
                    <div class="detail-item">
                        <label>Type:</label>
                        <span class="detail-value">${
                            attrs[config.fieldNames.type] || "Not specified"
                        }</span>
                    </div>
                </div>
            </div>
            ...
            <div class="details-section">
                <h3 class="section-title">🕒 Edit History</h3>
                <div class="detail-group">
                    <div class="detail-item">
                        <label>Last Editor:</label>
                        <span class="detail-value">${
                            attrs[config.fieldNames.editor] || "Not specified"
                        }</span>
                    </div>
                    <div class="detail-item">
                        <label>Last Edit Date:</label>
                        <span class="detail-value">${
                            attrs[config.fieldNames.editDate]
                                ? new Date(attrs[config.fieldNames.editDate]).toLocaleString()
                                : "Not specified"
                        }</span>
                    </div>
                </div>
        ...
        document
            ...
            .addEventListener("click", saveAccountUpdate);
        document
            .getElementById("cancelEdit")
            .addEventListener("click", closeMeterDetails);
    }
        ...
        try {
            ...
            if (result.updateFeatureResults && result.updateFeatureResults.length > 0) {
                ...
                if (updateResult.success) {
                    ...
                    const localFeature = allFeatures.find(
                        (f) => f.attributes.OBJECTID === selectedFeature.attributes.OBJECTID
                    );
    ...
    function showToast(message, type = "info") {
        ...
        toast.innerHTML = `
            <div class="toast-content">
                <span class="toast-icon">${type === "success"
                    ? "✅"
                    : type === "error"
                    ? "❌"
                    : "ℹ️"}</span>
